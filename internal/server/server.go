...
func CreateAutoCertManager(domain, email, certDir string) *autocert.Manager {
	...
	if err := os.MkdirAll(certDir, 0755); err != nil {
		log.Error("创建证书目录失败", "dir", certDir, "err", err)
		return nil
	}

	log.Info("配置自动证书管理", "domain", domain, "email", email, "certDir", certDir)
...
func StartWithAutoCert(srv *http.Server, domain, email, certDir string) error {
	...
	// 启动HTTP挑战服务器（端口80）
	go func() {
		log.Info("启动HTTP挑战服务器在端口80")
		if err := http.ListenAndServe(":80", m.HTTPHandler(nil)); err != nil {
			log.Error("HTTP挑战服务器错误", "err", err)
		}
	}()

	log.Info("启动HTTPS服务器", "addr", srv.Addr, "domain", domain)
...
func startTLSServer(srv *http.Server, cfg *config.Configuration) error {
	if cfg.TLS.AutoCert && cfg.TLS.Domain != "" {
		// 自动证书模式
		log.Info("启动自动HTTPS服务", "addr", cfg.Addr, "domain", cfg.TLS.Domain)
		return StartWithAutoCert(srv, cfg.TLS.Domain, cfg.TLS.Email, cfg.TLS.CertDir)
	}

	if cfg.TLS.CertFile != "" && cfg.TLS.KeyFile != "" {
		// 手动证书模式
		log.Info("启动HTTPS服务", "addr", cfg.Addr)
		return srv.ListenAndServeTLS(cfg.TLS.CertFile, cfg.TLS.KeyFile)
	}

	// 配置不完整，回退到HTTP
	log.Error("TLS配置错误: 需要配置自动证书(auto_cert+domain)或手动证书(cert_file+key_file)")
	log.Info("TLS配置不完整，回退到HTTP服务", "addr", cfg.Addr)
...
func StartServer(srv *http.Server) error {
	...
	if !cfg.TLS.Enabled {
		log.Info("启动HTTP服务", "addr", cfg.Addr)
		return srv.ListenAndServe()
	}
